    ...
    if len(lcount) == len(rcount):
        if lcount.most_common()[0][1] == rcount.most_common()[0][1]:
            for lchar, rchar in zip(left, right):
                if lchar != rchar:
                    return ((order.index(rchar) - order.index(lchar))) // abs(
                        ((order.index(rchar) - order.index(lchar)))
                    )
        return (lcount.most_common()[0][1] - rcount.most_common()[0][1]) // abs(
            (lcount.most_common()[0][1] - rcount.most_common()[0][1])
        )
    return (len(rcount) - len(lcount)) // abs((len(rcount) - len(lcount)))
